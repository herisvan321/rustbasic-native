...
fn main() {
    ...
    match args[1].as_str() {
        "install" => {
            make_native_scaffolding();
        }
        "uninstall" => {
            remove_native_scaffolding();
        }
        _ => {
...
fn make_native_scaffolding() {
    ...
    write_scaffold_file("native/run-android.sh", TEMPLATE_RUN_ANDROID_SH.as_bytes(), true);
    write_scaffold_file("native/build-android.sh", TEMPLATE_BUILD_ANDROID_SH.as_bytes(), true);
    write_scaffold_file("native/run-desktop.sh", TEMPLATE_RUN_DESKTOP_SH.as_bytes(), true);
    write_scaffold_file("native/native-bridge.js", TEMPLATE_NATIVE_BRIDGE_JS.as_bytes(), false);

    // Desktop wrapper
    write_scaffold_file("native/desktop/Cargo.toml", TEMPLATE_DESKTOP_CARGO_TOML.as_bytes(), false);
    write_scaffold_file("native/desktop/src/main.rs", TEMPLATE_DESKTOP_MAIN_RS.as_bytes(), false);
    write_scaffold_file("native/desktop/build.rs", TEMPLATE_DESKTOP_BUILD_RS.as_bytes(), false);
...
fn modify_user_cargo_toml() {
    ...
    if content.contains("websocket") && content.contains("rustbasic-core") {
        return; // already has websocket feature
    }

    println!("   {} Menambahkan fitur 'websocket' di Cargo.toml...", "📝".bold());
    ...
    if let Some(pos) = content.find("rustbasic-core") {
        if let Some(features_pos) = content[pos..].find("features = [") {
            ...
            content.insert_str(actual_features_pos, "\n    \"websocket\",");
...
### 2. Android
Untuk membuild library native saja:
```bash
...
const TEMPLATE_RUN_ANDROID_SH: &str = r##"#!/bin/bash
set -e
...
const TEMPLATE_BUILD_ANDROID_SH: &str = r##"#!/bin/bash
set -e
...
const TEMPLATE_RUN_DESKTOP_SH: &str = r##"#!/bin/bash
...
const TEMPLATE_DESKTOP_CARGO_TOML: &str = r##"[package]
name = "rustbasic-native-desktop"
version = "0.1.0"
edition = "2024"

[dependencies]
rustbasic = { path = "../../", version = "0.1" }
rustbasic-core = { path = "../../../rustbasic-core", version = "0.1", features = ["desktop", "websocket"] }
...
const TEMPLATE_DESKTOP_BUILD_RS: &str = r##"fn main() {
    #[cfg(target_os = "macos")]
    {
...
fn main() {
    ...
    let window = WindowBuilder::new()
        .with_title("RustBasic Native Desktop")
